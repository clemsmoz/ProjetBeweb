...
# route de l'accueil visiteur
@app.route('/')
def log():
    return render_template('login.html')

# route pour la verification de connexion
@app.route('/verifyLogin', methods=['GET'])
def login():
    if request.method == 'GET':
        user = verifyLog()
        if user != None:
            message = "bienevenu " + user.get_full_name()
...
# route pour affichage de l'acceuil et de la nav en fonction du role
@app.route('/accueil')
def index():
